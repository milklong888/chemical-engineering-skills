...
def build_model_recommendation(
    ...
    explicit_terminal_type = (
        explicit_type
        if explicit_type and token(explicit_type) not in generic_identity_tokens
        else ""
    )
    ...
    generic_candidate = {
        "candidate_id": f"{family_id}:engineering-designation",
        "rank": 1,
        # This row is generated from the family rule and current process/design
        # parameters only.  It is deliberately not a standard marking or vendor
        # candidate, even when the family ultimately needs that evidence class.
        "candidate_kind": "generic_type_placeholder",
        ...
        "missing_gates": sorted(set(missing_candidate_fields + model_blockers)),
        "is_vendor_model": False,
        "formal_model": False,
        "candidate_eligibility": (
    ...
    if supplied_model:
        ...
        candidates.append({
            ...
            "formal_model": model_status == "final_model",
            "candidate_eligibility": (
                "FORMAL_READY" if model_status == "final_model" else "EVIDENCE_OPEN"
            ),
            "eligible_for_formal_selection": model_status == "final_model",
            ...
            "source": {"kind": "user_supplied_unverified_candidate", **knowledge_route},
    ...
    if npsh_constraint_failed:
        ...
        for candidate in candidates:
            if candidate is generic_candidate:
