...
本树只导航[临时收件](EXPERIENCE_INBOX.md)、[进化流程](EVOLUTION_LOOP.md)、
[蒸馏流程](KNOWLEDGE_DISTILLATION_PIPELINE.md)及[严格准入](STRICT_ACCEPTANCE_AND_LEARNING.md)，
不重写其资格/质量判据。

| 判断 | 路径 |
|---|---|
| 明确的代码修复/外部来源维护 | 直接维护及来源审查，不伪造无关任务关闭 |
| 尚未关闭任务的表现或纠正 | 先修当前任务并留审计；有明确公开授权时按临时收件入口核验，尚不晋升正式规则 |
| 已确认例外分支或后代 | 仅项目审计；摘要、负例、反思和复跑不改变谱系 |
| 跑通的经验或已确认思想准备提交 | 分别检查真实运行范围，或同内容用户确认后的assistant审核；缺证据/授权为pending，临时箱不进默认RAG |
| 用户明确发起周期整理 | 固定本批内容；正式晋升仍经当前任务关闭、原谱系与质量门 |
