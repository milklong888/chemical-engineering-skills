...
压力容器项目文件的字段、偏移或材料字节审查先确认生成软件、版本和模块。身份未知时，走通用映射审查分支：形成风险表，列文件格式/版本、字段语义、受控副本、原软件读回及其生成报告的证据缺口，不指名要求SW6计算书。

当前身份确认是SW6后，才进入SW6分支：由设备审计核对同一设备的结构、设计条件、材料和证据等级，再审用户提供的已验证字段映射，将能证明安全序列化的字段写入受控副本，逐项回读比较单位、数值和文件身份，并用同一设备的SW6计算书核验正式结论。两个分支都不猜写未验证的材料、字符串或几何；缺执行证据时交付已完成账本和具体待补项，不声称有未执行的软件结果。
...
Read the current equipment identity, source ledger, allowed edits, producing
application, file format, version and module before scripting. An optional local equipment graph under
...
Choose the applicable branch from current evidence:

- **Unidentified or other producing application**: perform only the requested
  generic mapping-risk review. Return known identity facts and missing format,
  version, field semantics, protected-copy, serialization, independent readback
  and producing-application report evidence. Do not request an SW6 report, use
  SW6 field names/configuration or infer SW6 from a similar vessel/module name.
  A separately identified application's implementation belongs to its actual
  owner; missing identity does not prevent the generic risk review.
- **Confirmed SW6**: apply the SW6 implementation workflow below within the
  authorized scope. Record the identity evidence before its first specific step.
...
## SW6 Workflow — Confirmed SW6 Only
...
same-version sample comparison and readback through the identified producing
application prove serialization. An SW6 export is relevant only to the confirmed
SW6 branch.
