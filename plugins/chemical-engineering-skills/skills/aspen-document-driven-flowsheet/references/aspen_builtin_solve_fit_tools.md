...
先确定这是解释工具，还是在当前流程条件下制定求解方案。对于已有塔、反应器
等岛的目标闭合、操作点研究或优化方案，执行[阶段调用](../../chemical-engineering-expert/references/DESIGN_STAGE_ROUTING.md)
中的 `island`；只问“下一步怎么做”、禁止启动商业软件或尚缺模型文件，均不
取消已有资料支持的本地知识准备。新路线、已经发生的工况变化等按实际事件
选择相应阶段，不能看到优化意图就替用户编造新变更。纯原理、名称比较、读值
或一次换算保留原有窄范围。

没有本次同条件的阶段回执时，先查看运行库 `tools/expert_cli.py --schema design-stage`，
CLI 用 `--request 请求文件 --output 新回执文件`。岛方案的最小请求为
`{"operation":"design_stage","payload":{"stage":"island","question":"当前任务实际内容"}}`，
其中问题必须替换为当前原意，不填虚构模型、设备或授权。该最小结构进行本地
知识准备，不打开或运行 Aspen、EDR、SW6；具体数据缺口保留在回执中。
读取摘要、缺口及实际知识命中，说明哪些依据适用于本题、哪些仍不确定，再交
求解方案。已在当前阶段入口得到同条件回执时直接复用，不因嵌套路由待办反复
调用同一阶段。塔方案还须读取[塔优化工作流](../../aspen-tower-optimization-workflow/SKILL.md)，
保留原约束与内外层变量的职责；路由分类本身不替代这项专业交接。
