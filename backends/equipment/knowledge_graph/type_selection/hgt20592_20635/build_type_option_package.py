...
def main():
    parser = argparse.ArgumentParser(description="Original HG/T builder with explicit user-owned source and output roots.")
    parser.add_argument("--source-root", type=Path)
    parser.add_argument("--output-root", type=Path)
    args = parser.parse_args()
    global ROOT, SOURCE_ROOT, RAW_PAGES
    if args.source_root is None or args.output_root is None or not (args.source_root / "raw_pages.jsonl").is_file():
        print(json.dumps({"status": "dependency_unavailable", "required": ["--source-root containing raw_pages.jsonl", "--output-root"], "outputs_created": False}))
        return 2
    source = args.source_root.resolve()
    output = args.output_root.resolve()
    backend = Path(__file__).resolve().parents[3]
    if output.exists() or output.is_relative_to(backend) or output.is_relative_to(source) or source.is_relative_to(output):
        raise ValueError("Choose a new output root outside the source and installed backend")
    SOURCE_ROOT, RAW_PAGES = source, source / "raw_pages.jsonl"
    pages = load_pages()
    if any(pages[p]["source_pdf_sha256"] != PDF_SHA256 for p in EVIDENCE_PAGES):
        raise ValueError("source PDF hash mismatch")
    output.mkdir(parents=True)
    for file in ROOT.iterdir():
        if file.is_file() and file.name not in {"hash_manifest.csv", "SOURCE_PROJECTION.json", "source_records.csv"}:
